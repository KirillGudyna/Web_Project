...
<fmt:bundle basename="property.gui">
    <head>
        <title><fmt:message key="doctor.open.appointments.title"/></title>
        <link type="text/css" href="css/bootstrap.css" rel="stylesheet"/>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    </head>
    <body>
        <c:set var="appointments" value="${appointments}" scope="request"/>
        <c:choose>
               <c:when test="${appointments.size() > 0}">
                   <form action="ProcessController" method="POST" id="choose_appointment">
                       <h2><fmt:message key="table.open.appointments.caption"/></h2>
                       <table class="table table-striped table-hover table-bordered">
                           ...
                           <c:forEach var="item" items="${appointments}">
                               <tr>
                                   <td>
                                       <input type="radio" name="chosen_appointment" value="${item.getId()}" checked/>
                       ...
                       <input  type="submit" class="btn btn-primary" formaction="ProcessController?command=choose_appointment"
                               value=<fmt:message key="choose_appointment.button.choose"/>/>
                       <input class="btn btn-success" formaction="ProcessController?command=go_to_add_purpose_page" type="submit"
                              value=<fmt:message key="appointment.button.add.purpose"/>/>
                   </form>
               </c:when>
               <c:otherwise>
                   <h3><fmt:message key="no.open.appointments.message"/></h3>
               </c:otherwise>
           </c:choose>
